...
SystemMonitorGraph.prototype = {
    ...
    _init: function(metadata, desklet_id) {
		...
		this.settings.bindProperty(Settings.BindingDirection.IN, "temp-label", "temp_label", this.on_setting_changed);
    ...
    update_draw: function() {
        ...
        switch (this.type) {
          ...
          case "temp":
              ...
              let max_temp = 100; // Maximum temperature for graph scaling
              value = Math.min(this.temp_value, max_temp) / max_temp;
              text1 = _("TEMP");
              text2 = this.temp_value.toFixed(1) + "°C";
              text3 = this.temp_label;
              break;
        }
	...
	get_temperature: function() {
	    try {
	        let file_path = decodeURIComponent(this.temp_type.replace("file://", "").trim());
	        let file = Gio.File.new_for_path(file_path);
	        file.load_contents_async(null, (file, response) => {
	            try {
	                let [success, contents, tag] = file.load_contents_finish(response);
	                if (success) {
	                    let content = ByteArray.toString(contents).trim();
	                    let temp_raw = parseFloat(content);
	                    // Apply scaling (millidegrees to degrees if needed)
	                    let temp_scaled = temp_raw / this.temp_scale;
	                    this.temp_value = temp_scaled;
	                } else {
	                    this.temp_value = 0;
	                }
	                GLib.free(contents);
	                ...
	                this.temp_value = 0;
	                global.logError("Failed to parse temperature: " + e.toString());
	            }
	        });
	    } catch (e) {
	        this.temp_value = 0;
	        global.logError("Temperature file error: " + e.toString());
	    }
	},
